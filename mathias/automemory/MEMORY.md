...
- [Native <datalist> limits](project_native-datalist-limits.md) — the popup is browser UI: colour follows the Firefox theme and placement is Chrome's, neither reachable from page CSS; hiding Chrome's own arrow needs `!important`, and it only shows on hover.
- [web-ui frontend constraints](project_web-ui-frontend-constraints.md) — no npm/webjars/CDN; the layout pulls only `~{::section}`, so markup placed outside it is silently dropped; scope data-loading `@ControllerAdvice` with `assignableTypes`.
